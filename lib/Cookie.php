<?php
// from http://stut.net/2008/10/16/snippet-cookie-class-for-php/
//@modif niahoo ajout de $secure et $httponly dans ::Set
//@modif ajout de la fonction SetOpts
class Cookie
{
  const Session = null;
  const OneDay = 86400;
   ...
   * @return bool
   */
  static public function Set($name, $value, $expiry = self::OneYear, $path = '/', $domain = false, $secure = false, $httponly = false)
  {
    $retval = false;
    if (!headers_sent())
    {
      ...
      if ($expiry === -1)
        $expiry = 1893456000; // Lifetime = 2030-01-01 00:00:00 @todo
...
static public function SetOpts($name, $value, $_opts=array()) {
  $defaults = array(
    'expiry' => self::OneYear,
    'path' => '/',
    'domain' => false,
  ...
  $opts = array_merge($defaults, $_opts);
  extract($opts);

  return self::Set($name, $value, $expiry, $path, $domain, $secure, $httponly);
}
